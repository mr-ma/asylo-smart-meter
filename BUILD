...
cc_unsigned_enclave(
    name = "smart_enclave_unsigned.so",
    srcs = ["smart_enclave.cc"],
    deps = [
        ":smart_cc_proto",
...
debug_sign_enclave(
    name = "smart_enclave.so",
    unsigned = "smart_enclave_unsigned.so",
)

enclave_loader(
    name = "smart_meter",
    srcs = ["smart_driver.cc"],
    backends = sgx.backend_labels,  # Has SGX loader dependencies
    enclaves = {"enclave": ":smart_enclave.so"},
